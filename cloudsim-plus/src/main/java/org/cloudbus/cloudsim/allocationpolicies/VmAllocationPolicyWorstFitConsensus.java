...
public class VmAllocationPolicyWorstFitConsensus extends VmAllocationPolicyAbstract {

    @Override
    protected Optional<Host> defaultFindHostForVm(final Vm vm) {
    	
		int maxTries = getHostList().size();
	...
	private Optional<Host> recFindHostForVm(Vm vm, Stream<Host> stream, int maxTries, Comparator<Host> comparator){
        ...
        Optional<Host> opHost = stream.filter(host -> host.isSuitableForVm(vm)).max(comparator);
        if(opHost.isPresent()) {
